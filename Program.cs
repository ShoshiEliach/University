...
class Program
{
    ...
    static void Main(string[] args)
    {
        ...
        //create the person(proffesors and students) list.
        string[] departments = { "Lecturer", "Senior Lecturer", "Associate Professor", "Professor" };
        for (int i = 0; i < namesProffesor.Length; i++)
        {
            Random random = new Random();
            Person p = new Professor(namesProffesor[i], random.Next(50, 71), departments[random.Next(departments.Length)], addressProffesor[i]);
            personList.Add(p);
        }
        ...
        //add the participants to the courses
        List<Person> tempersonList= new List<Person>();
       tempersonList = personList;
